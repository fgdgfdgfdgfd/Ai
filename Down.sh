...
if command -v docker &> /dev/null; then
    ...
    log "📦 Загружаем и устанавливаем Docker..."
...
if docker compose version > /dev/null 2>&1; then
    ...
    # Compose уже включен в docker-ce, но на всякий случай
...
if command -v ollama &> /dev/null; then
    warn "Ollama уже установлена"
    ollama --version || true
else
    log "📦 Загружаем и устанавливаем Ollama..."
...
# Создаем системный сервис для Ollama
...
# Определяем IP хоста для связи с Ollama
...
while true; do
    ...
    case $choice in
        ...
        5) ollama list; read -p "Нажмите Enter для продолжения..."; ;;
        6)
...
echo "• Логи находятся в: /tmp/ollama.log и docker logs"
